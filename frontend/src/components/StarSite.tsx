  ...
  return (
    <header className={floating ? "absolute inset-x-0 top-0 z-30" : "relative z-30"}>
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 sm:px-8 lg:px-10">
        ...
        <nav className="hidden items-center gap-12 text-sm font-semibold text-starMist/78 md:flex">
          <Link className="transition hover:text-starGold" href={ROUTES.home}>
            首页
          </Link>
          <Link className="transition hover:text-starGold" href={ROUTES.dashboard}>
            星光故事
          </Link>
          <Link className="transition hover:text-starGold" href={ROUTES.dashboard}>
            我的星空
