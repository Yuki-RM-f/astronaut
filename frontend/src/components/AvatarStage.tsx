...
import { AvatarPreview, type AvatarModelLoadState } from "@/src/components/AvatarPreview";
import type { AvatarDisplaySource } from "@/src/lib/avatar";
import type { MotionIntent } from "@/src/lib/gesture";
...
export function AvatarStage({
  ...
  subtitle?: string;
  onModelLoadStateChange?: (state: AvatarModelLoadState) => void;
  className?: string;
}) {
  return (
    <aside
      ...
      <div className="relative flex h-full min-h-[32rem] flex-col justify-between gap-6 p-5">
        <div className="min-h-[23rem]">
          {source.kind === "model" ? (
            <AvatarPreview
              ...
              motionIntent={motionIntent}
              onLoadStateChange={onModelLoadStateChange}
        ...
        <div className="relative rounded-3xl border border-white/10 bg-black/18 p-4">
          <p className="inline-flex items-center gap-2 text-sm font-bold text-starGold">
            ...
            {source.kind === "placeholder" ? "未上传 GLB 模型" : "已连接 GLB 模型"}
          </p>
          ...
          <p className="mt-3 text-sm font-semibold leading-7 text-starMist/72">
            {subtitle || "上传 GLB 后，数字人模型会出现在这里，继续陪你对话。"}
          </p>
