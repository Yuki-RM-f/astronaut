...
def get_audit_summary(db: Session, persona_id: str) -> dict:
    ...
    unacknowledged_drifts = (
        db.scalar(
            select(func.count(PersonaDrift.id)).where(
                PersonaDrift.persona_id == persona_id,
    ...
    return {
        ...
        "unacknowledged_drifts": unacknowledged_drifts,
        "last_snapshot_at": last_snapshot.created_at.isoformat() if last_snapshot else None,
        "last_snapshot_id": last_snapshot.id if last_snapshot else None,
    }


def generate_audit_report(db: Session, persona_id: str) -> dict:
    from app.services.profile import calculate_trust_report
    ...
    drifts = list(
        db.scalars(
            select(PersonaDrift)
            .where(PersonaDrift.persona_id == persona_id)
            .order_by(PersonaDrift.created_at.desc())
            .limit(50)
        ).all()
    )
    ...
    trust_trend = [
        {"at": s.created_at.isoformat(), "score": s.trust_score}
        for s in reversed(snapshots)
    ]

    current_trust = calculate_trust_report(db, persona) if persona else None
    ...
    def _drift_to_dict(d: PersonaDrift) -> dict:
        return {
            "id": d.id,
    ...
    return {
        ...
        "current_trust": current_trust.model_dump() if current_trust else None,
        "source_coverage": {
...
def create_snapshot(
    ...
    *,
    ...
    persona = db.get(Persona, persona_id)
    profile = db.scalar(
        select(PersonaProfile).where(PersonaProfile.persona_id == persona_id)
    )
    memories = list(
        db.scalars(
    ...
    trust = calculate_trust_report(db, persona) if persona else None

    snapshot = AuditSnapshot(
        persona_id=persona_id,
        snapshot_type=snapshot_type,
        label=label,
        persona_snapshot=snapshot_entity(persona) if persona else {},
        profile_snapshot=snapshot_entity(profile) if profile else {},
        memory_snapshots=[snapshot_entity(m) for m in memories],
        trust_report=trust.model_dump() if trust else {},
...
def compare_snapshots(
    db: Session, snapshot_id_a: str, snapshot_id_b: str
) -> dict:
    a = db.get(AuditSnapshot, snapshot_id_a)
    b = db.get(AuditSnapshot, snapshot_id_b)
    if not a or not b:
    ...
    return {
        ...
        "trust_score_delta": b.trust_score - a.trust_score,
        "memory_count_delta": b.memory_count - a.memory_count,
        "persona_changes": diff_before_after(a.persona_snapshot, b.persona_snapshot),
        "profile_changes": diff_before_after(a.profile_snapshot, b.profile_snapshot),
    }


def get_snapshots(
    db: Session, persona_id: str, limit: int = 20
) -> list[AuditSnapshot]:
    return list(
        db.scalars(
            select(AuditSnapshot)
...
def rollback_to_snapshot(
    ...
    snapshot = db.get(AuditSnapshot, snapshot_id)
    if not snapshot:
        raise ValueError(f"Snapshot {snapshot_id} not found")

    persona = db.get(Persona, snapshot.persona_id)
    if not persona:
        raise ValueError(f"Persona {snapshot.persona_id} not found")

    safety = create_snapshot(
        db,
        persona_id=persona.id,
        snapshot_type="pre_rollback",
        label=f"Auto-saved before rollback to {snapshot.label or snapshot_id}",
        commit=False,
    )

    persona_before = snapshot_entity(persona)
    for field, value in snapshot.persona_snapshot.items():
        if field not in ("id", "created_at", "updated_at", "deleted_at"):
    ...
    if profile and snapshot.profile_snapshot:
        for field, value in snapshot.profile_snapshot.items():
            if field not in ("id", "persona_id", "created_at", "updated_at"):
    ...
    for mem_dict in snapshot.memory_snapshots:
        mem_dict_copy = dict(mem_dict)
        mem_dict_copy.pop("id", None)
        for key, value in list(mem_dict_copy.items()):
            if value is not None and isinstance(value, str) and key.endswith("_at"):
                try:
                    mem_dict_copy[key] = datetime.fromisoformat(value)
                except (ValueError, TypeError):
                    pass
        new_mem = MemoryCard(**mem_dict_copy)
        db.add(new_mem)
    ...
    log = write_audit_event(
        ...
        user_id=user_id,
        persona_id=persona.id,
        target_type="audit_snapshot",
        target_id=snapshot_id,
        event_type="rollback.executed",
        severity="critical",
        action=f"Rolled back to snapshot '{snapshot.label or snapshot_id}'",
        before_snapshot={"persona": persona_before},
        correlation_id=correlation_id,
        metadata_json={"safety_snapshot_id": safety.id},
        commit=True,
    )
    return log
...
def get_audit_dashboard(db: Session, persona_id: str) -> dict:
    ...
    traceable = sum(
        ...
        if m.source_quote and m.source_location
    )
    ...
    return {
        ...
        "unacknowledged_drift_count": summary["unacknowledged_drifts"],
