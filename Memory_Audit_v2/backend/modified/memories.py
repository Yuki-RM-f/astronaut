...
from app.services.drift_detector import check_drift_after_profile_change
...
def create_memory(
    ...
    write_audit_event(
        ...
        severity="info",
        action=f"Memory '{memory.title}' created ({memory.status})",
        after_snapshot=snapshot_entity(memory),
    )
    detect_conflicts_for_memory(db, memory)
    check_drift_after_profile_change(db, persona.id)
...
def update_memory(
    ...
    write_audit_event(
        ...
        severity="warning",
        action=f"Memory '{memory.title}' edited",
        before_snapshot=before,
        after_snapshot=after,
        changed_fields=list(updates.keys()),
    )
    detect_conflicts_for_memory(db, memory)
    check_drift_after_profile_change(db, memory.persona_id)
...
def _set_memory_status(
    ...
    event_type = status_event_map.get(new_status, "memory.updated")
    severity = "warning" if new_status in ("rejected",) else "info"
    ...
    write_audit_event(
        ...
        severity=severity,
        action=f"Memory '{memory.title}' status → {new_status}",
        before_snapshot=before,
        after_snapshot=snapshot_entity(memory),
...
def delete_memory(
    ...
    write_audit_event(
        ...
        severity="critical",
        action=f"Memory '{memory.title}' deleted",
        before_snapshot=before,
    )
    check_drift_after_profile_change(db, memory.persona_id)
