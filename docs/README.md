...
补充：2026-07-05 Dashboard 默认双人物已改为后端确定性私有种子。`GET /api/personas` 会在查询前为当前用户幂等补齐 active 的「外婆」（关系为外婆）和「郑木生」（关系为爷爷）；demo 用户、正式注册用户和既有用户都会得到各自私有副本。用户软删除默认人物后，下次进入 Dashboard 或再次请求人物列表会重新创建新的 active 副本。该 seed 直接写入 `Persona`、`SourceMaterial(parse_status=succeeded)`、`ParsedChunk`、已确认 `MemoryCard`、`PersonaProfile` 和 trust，不在 Dashboard 请求中调用真实 LLM/provider；`POST /api/auth/demo` 复用同一 seed 服务，`demo_persona_id` 继续返回「外婆」的 id。
...
补充：遗憾对话室和心愿延续页在空对话开场时会先调用 `POST /api/personas/{id}/guided-memory-candidates`，从当前用户当前人物已审核、未删除的 `MemoryCard` 中提取最多三条遗憾/心愿候选；MiniMax/OpenAI-Next 文本能力可用时使用 `guided_memory_extraction`，未配置或失败时使用确定性关键词兜底。前端仅在当前 guided conversation 尚无消息且存在候选时展示候选卡片，点击只把建议文案填入输入框并记录 `guided_memory_ids`，不会自动发送；用户发送后，后端只把这些被选中的来源记忆纳入本轮 guided chat 上下文和 citations。无候选或用户未选择候选时，仍保持原有引导输入和上下文隔离。
