...
当前仓库已完成 Milestone 0 基础工程、Milestone 1 人物创建/记忆空间基础闭环、Milestone 2 资料上传/任务队列基础闭环、Milestone 3 mock 多模态解析/记忆审计基础闭环、Milestone 4 人格档案/可信度基础闭环、Milestone 5 第一人称文本对话基础闭环，Milestone 6 Task 1/2/3/4 后端默认 TTS、语音合成、音色样本、音色克隆兜底、语音消息 ASR 到 TTS、前端录音、声音设置和 mock 语音播放基础闭环，并已接入 Milestone 7 Task 1/2/3 后端 3D 数字人配置、默认形象、mock 生成、前端 Three.js mock 预览、对话页 3D 数字人展示和播放状态口型联动基础闭环、Milestone 8 后端回忆讲述生成和聊天页星空互动主界面/记忆档案馆入口，以及全站温馨沉浸式「记忆空间」前端与免注册一键演示入口：后端为 FastAPI，前端为 Next.js，基础依赖包含 PostgreSQL、Redis、MinIO 和 mock worker 进程；后端已提供用户隔离的人物 CRUD、`prompt_context` 输出、SourceMaterial 资料记录、AI Job 状态接口、本地 storage 兜底、deterministic mock Provider Gateway 解析、`ParsedChunk`/source-backed `MemoryCard` 生成、记忆审计 API、profile GET/PATCH/regenerate API、recalculate-trust API、conversation/message/citation/correct-memory/voice-message API、stories list/generate/favorite API、`POST /api/auth/demo` 本地演示会话 API、mock `chat_llm` Provider Gateway、voice config/default TTS/samples/clone/synthesize API、avatar config/default/generate API，以及 mock `tts`/`extract_voice_sample`/`voice_clone`/`avatar_3d` Provider Gateway；前端已接入中文首页、免注册演示入口、注册/登录表单、人物列表、四步创建人物、人物记忆空间页、资料上传页、任务状态页、记忆审计页、人格档案/可信度页、星空互动对话页（含文字互动、视频手势互动入口、语音互动入口、记忆档案馆故事生成入口、selected mock 3D 数字人与播放状态口型联动）、声音设置页、3D 形象设置页和本地 Pexels 视觉素材。本文只描述当前已存在的代码事实和验证入口；真实 OCR/ASR/VLM/模型质量、真实 LLM/embedding 检索质量、真实 GLB/3D provider 质量、独立故事管理页、故事导出和删除关键数据仍未实现。
...
- Milestone 7 目前完成后端 avatar config/default/generate/mock fallback API、前端 Three.js mock 头像/半身预览和聊天页 selected mock 数字人播放状态口型联动；Milestone 8 目前完成后端回忆讲述生成和聊天页记忆档案馆入口；真实 GLB 加载、真实音频音量包络/viseme 口型同步、真实 3D provider 质量、独立故事管理页、故事导出、删除关键数据和真实 LLM/embedding 检索质量仍未实现。
